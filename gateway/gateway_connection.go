...
func EstablishConnection(ctx context.Context) error {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 10 ] sending initial request to Discord Gateway server", utils.GetCurrTimeUTC()))
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 10 ] value of heartbeat interval is: %d seconds", utils.GetCurrTimeUTC(), (heartbeat_interval / 1000)))
...
func ReceiveMessage(connection *websocket.Conn) (int, *int, error) {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 10 ] received: %s\n", utils.GetCurrTimeUTC(), msg))
...
func SendMessage(connection *websocket.Conn, sequence_num *int) error {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 01 ] sending the following payload: {op: %d, d: {Seq: %v}}", utils.GetCurrTimeUTC(), op_1_heartbeat.OP, op_1_heartbeat.D.Sequence))
...
func ACK(connection *websocket.Conn) error {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 11 ] received: %s\n", utils.GetCurrTimeUTC(), msg))
...
func Identity(connection *websocket.Conn) error {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 02 ] sending Identity payload", utils.GetCurrTimeUTC()))
...
func Ready(connection *websocket.Conn) error {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 0 ] reading message returned from server", utils.GetCurrTimeUTC()))
	_, msg, err := connection.ReadMessage()
	if err != nil {
	...
	logrus.Info(fmt.Printf("%s: [ OP CODE 0 ] Received: %s", msg, utils.GetCurrTimeUTC()))
